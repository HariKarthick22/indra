...
cargo build --release -p indra-cli --bin indra --no-default-features --features portable-default
```
...
1. Built the Rust binary: `cargo build --release -p indra-cli --bin indra`
...
1. **Backend changes**: Rebuild with `cargo build --release -p indra-cli --bin indra` and copy the binary
