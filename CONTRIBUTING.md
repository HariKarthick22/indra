...
These same commands can be recompiled and immediately run using `cargo run -p indra-cli` for iteration.
...
cargo run --package indra-cli --bin indra -- serve --platform desktop --enable-scheduler --host 127.0.0.1 --port 3000
```
...
GOOSE_PATH_ROOT="/tmp/indra-dev" cargo run -p indra-cli -- session
```
