...
impl From<indra_providers::errors::ProviderError> for GooseError {
    fn from(error: indra_providers::errors::ProviderError) -> Self {
        match error {
            indra_providers::errors::ProviderError::Authentication(message) => {
                Self::Authentication { details: message }
            }
            indra_providers::errors::ProviderError::ContextLengthExceeded(message) => {
                Self::ContextLengthExceeded { details: message }
            }
            indra_providers::errors::ProviderError::RateLimitExceeded { retry_delay, .. } => {
            ...
            indra_providers::errors::ProviderError::ServerError(message)
            | indra_providers::errors::ProviderError::EndpointNotFound(message)
            | indra_providers::errors::ProviderError::CreditsExhausted {
                details: message, ..
            } => Self::ProviderUnavailable { details: message },
            indra_providers::errors::ProviderError::NetworkError(message)
                if is_timeout(&message) =>
            {
                Self::Timeout { details: message }
            }
            indra_providers::errors::ProviderError::RequestFailed(message)
                if is_timeout(&message) =>
            {
                Self::Timeout { details: message }
            }
            indra_providers::errors::ProviderError::ExecutionError(message)
...
impl indra_providers::request_log::RequestLogger for RequestLoggerAdapter {
    fn start(
        &self,
    ) -> Result<
        Box<dyn indra_providers::request_log::RequestLogHandle>,
...
impl indra_providers::request_log::RequestLogHandle for RequestLogHandleAdapter {
...
pub fn install_request_logger(logger: Box<dyn RequestLogger>) -> Result<(), GooseError> {
    indra_providers::request_log::install_logger(RequestLoggerAdapter {
...
impl Provider {
    ...
    pub async fn compact(
        ...
        let model = indra_context_management::ProviderModel::new(
            self.handle.provider.clone(),
            ModelConfig::new(&model_name),
        );

        let summary = run_on_runtime(async move {
            indra_context_management::summarize(&model, None, &templates, &messages).await
        })
        .await?
        .map_err(GooseError::generic)?;
...
impl From<CompactionTemplates> for indra_context_management::Templates {
...
pub fn default_compaction_templates() -> CompactionTemplates {
    let templates = indra_context_management::Templates::default();
    CompactionTemplates {
...
pub fn declarative_provider_from_json(json: String) -> Result<Arc<Provider>, GooseError> {
    let provider = indra_providers::declarative::from_json(&json, None, EnvKeyResolver {})?;
    Ok(Provider::new(provider))
}

#[uniffi::export]
pub fn openai_default_model() -> String {
    indra_providers::openai::OPEN_AI_DEFAULT_MODEL.to_string()
}
...
pub fn anthropic_default_model() -> String {
    indra_providers::anthropic::ANTHROPIC_DEFAULT_MODEL.to_string()
}
...
pub fn anthropic_provider(
    ...
    .with_header(
        "anthropic-version",
        indra_providers::anthropic::ANTHROPIC_API_VERSION,
    )?;
...
pub fn groq_provider(api_key: String) -> Result<Arc<Provider>, GooseError> {
    let json = indra_providers::groq::JSON.replace("${GROQ_API_KEY}", &api_key);
    let provider = indra_providers::declarative::from_json(
        &json,
        None,
        StaticKeyResolver {
...
fn groq_config() -> DeclarativeProviderConfig {
    indra_providers::declarative::deserialize_provider_config(indra_providers::groq::JSON)
...
impl indra_providers::declarative::KeyResolver for StaticKeyResolver {
...
pub fn databricks_default_model() -> String {
    indra_providers::databricks::DATABRICKS_DEFAULT_MODEL.to_string()
}
...
pub fn databricks_v2_default_model() -> String {
    indra_providers::databricks_v2::DATABRICKS_V2_DEFAULT_MODEL.to_string()
}
...
mod tests {
    ...
    fn provider_message_converts_document_serializes_for_anthropic() {
        ...
        let spec = indra_providers::formats::anthropic::format_messages(&[message]);
    ...
    fn usage_exposes_provider_additional_data() {
        let mut provider_usage = ProviderUsage::new(
            ...
            indra_providers::conversation::token_usage::Usage::new(Some(10), Some(5), None),
        );
    ...
    fn provider_usage_with_cache(
        ...
        use indra_providers::conversation::token_usage::Usage as GooseUsage;
    ...
    fn tool_request_round_trips_provider_metadata() {
        ...
        .unwrap();
        let spec = indra_providers::formats::openai::format_messages(
            &messages,
            &indra_providers::images::ImageFormat::OpenAi,
        );
        assert_eq!(
    ...
    fn streaming_tool_chunks_carry_provider_metadata() {
        let mut metadata = indra_providers::conversation::message::ProviderMetadata::new();
        metadata.insert(
    ...
    fn thinking_blocks_survive_multi_turn_replay() {
        ...
        let spec = indra_providers::formats::anthropic::format_messages(&messages);
