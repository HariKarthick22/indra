...
use anyhow::Result;
use indra::agents::{Agent, AgentConfig, GoosePlatform};
use indra::config::permission::PermissionManager;
use indra::config::GooseMode;
use indra::providers::{create, testprovider::TestProvider};
    ...
    F: Fn(&ScenarioResult) -> Result<()>,
{
    ...
    indra::agents::moim::SKIP.with(|f| f.set(true));
    ...
    let scenario_model_config =
        indra::model_config::model_config_from_user_config(&factory_name, config.model_name)?;
    agent
        .update_provider(
            provider_arc as Arc<dyn indra::providers::base::Provider>,
