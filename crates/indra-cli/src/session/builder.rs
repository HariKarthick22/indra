...
use console::style;
use indra::agents::{Agent, Container, ExtensionError};
use indra::config::extensions::name_to_key;
use indra::config::resolve_extensions_for_new_session;
use indra::config::{Config, ExtensionConfig, GooseMode};
use indra::model_config::model_config_from_user_config;
use indra::providers::create;
use indra::recipe::Recipe;
use indra::session::session_manager::SessionType;
use indra::session::EnabledExtensionsState;
...
async fn resolve_provider_and_model(
    ...
    saved_model_config: Option<indra_providers::model::ModelConfig>,
    ...
    let configured_provider_model = session_config.provider.as_ref().and_then(|_| {
        indra::config::get_provider_entry(config, &provider_name)
    ...
    {
        Some(
            indra::providers::get_from_registry(&provider_name)
                .await
                .unwrap_or_else(|e| {
            ...
            .is_some_and(|mc| mc.model_name == model_name)
    {
        ...
        config = indra::model_config::with_rederived_cache_ttl(config).unwrap_or_else(|e| {
...
async fn resolve_session_id(
    session_config: &SessionBuilderConfig,
    session_manager: &indra::session::session_manager::SessionManager,
...
async fn collect_extension_configs(
    ...
    if !session_config.no_profile && !session_config.resume && recipe_extensions.is_none() {
        ...
        all.extend(
            indra::plugins::mcp_servers::enabled_plugin_mcp_servers(project_root.as_deref())
                .into_iter()
                .map(|config| (config.name(), config)),
        );
    }
...
pub async fn build_session(session_config: SessionBuilderConfig) -> CliSession {
    ...
    indra::posthog::set_session_context("cli", session_config.resume);
...
mod tests {
    use super::*;
    use indra::config::{set_provider_entry, ProviderEntry};
    ...
    fn saved_model_config(model_name: &str) -> indra_providers::model::ModelConfig {
        indra_providers::model::ModelConfig::new(model_name).with_merged_request_params(
    ...
    fn test_session_builder_config_creation() {
        let config = SessionBuilderConfig {
            ...
            streamable_http_extensions: vec![StreamableHttpOptions {
                ...
                timeout: indra::config::DEFAULT_EXTENSION_TIMEOUT,
            }],
    ...
    async fn resume_provider_override_uses_target_provider_model() {
        ...
        let resolved = resolve_provider_and_model(
            ...
            Some(indra_providers::model::ModelConfig::new(
                "claude-sonnet-4-6",
            )),
        )
        .await;
    ...
    async fn matching_environment_model_overrides_saved_model() {
        ...
        let resolved = resolve_provider_and_model(
            ...
            Some(indra_providers::model::ModelConfig::new("saved-model")),
        )
        .await;
    ...
    async fn resume_provider_override_uses_target_provider_default_instead_of_active_model() {
        ...
        config
            ...
            .unwrap();
        let expected_model = indra::providers::get_from_registry("openai")
            .await
            .unwrap()
            .metadata()
            .default_model
    ...
    async fn resume_rederives_cache_ttl_from_config_not_session() {
        ...
        let config = test_config(&temp_dir);
        let saved =
            indra_providers::model::ModelConfig::new("claude-sonnet-4-6").with_cache_ttl("5m");
    ...
    async fn resume_drops_saved_cache_ttl_when_config_absent() {
        ...
        let config = test_config(&temp_dir);
        let saved =
            indra_providers::model::ModelConfig::new("claude-sonnet-4-6").with_cache_ttl("1h");
    ...
    async fn headless_resume_clamps_rederived_cache_ttl() {
        ...
        let temp_dir = TempDir::new().unwrap();
        let config = test_config(&temp_dir);
        let saved = indra_providers::model::ModelConfig::new("claude-sonnet-4-6");
