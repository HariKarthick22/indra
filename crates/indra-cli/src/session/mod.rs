...
use indra::conversation::{fix_conversation, merge_consecutive_messages_for_request, Conversation};
use std::io::Write;
use std::str::FromStr;
...
use indra::agents::platform_extensions::developer::shell::{
    parse_shell_output_notification, ShellOutputNotificationParams, ShellOutputStream,
};
use indra::agents::AgentEvent;
use indra::agents::SUBAGENT_TOOL_REQUEST_TYPE;
use indra::permission::Permission;
use indra::providers::base::Provider;
use indra::providers::base::ProviderUsage;
use indra::utils::safe_truncate;

use anyhow::{Context, Result};
use completion::GooseCompleter;
use indra::agents::extension::{Envs, ExtensionConfig, PLATFORM_EXTENSIONS};
use indra::agents::types::RetryConfig;
use indra::agents::{
    context_management_unsupported_message, Agent, SessionConfig, COMPACT_TRIGGERS,
};
use indra::config::extensions::name_to_key;
use indra::config::{Config, GooseMode};
...
use indra::config::paths::Paths;
use indra::config::providers;
use indra::conversation::message::{
    ActionRequiredData, Message, MessageContent, ToolConfirmationRequest,
};
use indra::providers::inventory::ProviderInventoryService;
use indra::session::SessionManager;
...
pub async fn classify_planner_response(
    ...
    model_config: indra_providers::model::ModelConfig,
    ...
    let (result, _usage) = indra::session_context::with_session_id(
...
impl CliSession {
    ...
    pub fn parse_stdio_extension(extension_command: &str) -> Result<ExtensionConfig> {
        ...
        let mut parts = indra::utils::split_command_args(command)?;
        ...
        Ok(ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: indra::config::DEFAULT_EXTENSION_DESCRIPTION.to_string(),
            timeout: Some(indra::config::DEFAULT_EXTENSION_TIMEOUT),
            cwd: None,
            bundled: None,
    ...
    pub async fn add_streamable_http_extension(&mut self, extension_url: String) -> Result<()> {
        let config = Self::parse_streamable_http_extension(
            &extension_url,
            indra::config::DEFAULT_EXTENSION_TIMEOUT,
        );
        self.add_and_persist_extensions(vec![config]).await
    }
    ...
    pub async fn interactive(&mut self, prompt: Option<String>) -> Result<()> {
        let banners = self
            .agent
            .emit_hook_with_banners(indra::hooks::HookEvent::SessionStart, &self.session_id)
            .await;
        if !banners.is_empty() {
        ...
        self.agent
            .emit_hook(indra::hooks::HookEvent::SessionEnd, &self.session_id)
            .await;
    ...
    async fn handle_model(&mut self, options: input::ModelCommandOptions) -> Result<()> {
        ...
        let target_entry = match indra::providers::get_from_registry(target_provider_name).await {
            Ok(entry) => entry,
            Err(_) => {
                output::render_error(&format!(
        ...
        let current_context_limit = indra::context_limit::get_context_limit(
            provider.as_ref(),
        ...
        let new_provider = match indra::providers::create(target_provider_name, extensions).await {
            Ok(p) => p,
            Err(e) => {
                output::render_error(&format!(
        ...
        let new_context_limit = indra::context_limit::get_context_limit(
            new_provider.as_ref(),
    ...
    async fn handle_clear(&mut self) -> Result<()> {
        ...
        if let Err(e) = self
            ...
            .usage(indra_providers::conversation::token_usage::Usage::new(
                Some(0),
                Some(0),
                Some(0),
            ))
    ...
    async fn handle_list_skills(&mut self) -> Result<()> {
        ...
        use indra::custom_requests::SourceType;
        use indra::skills::list_installed_skills;
    ...
    async fn plan_with_reasoner_model(
        ...
        output::show_thinking();
        let (plan_response, _usage) = indra::session_context::with_session_id(
    ...
    async fn refresh_completion_cache(
        ...
        let all_providers = indra::providers::providers().await;
    ...
    pub async fn get_session(&self) -> Result<indra::session::Session> {
        self.agent
            .config
            .session_manager
    ...
    pub async fn display_context_usage(&self) -> Result<()> {
        ...
        let model_config = self
            ...
            .await?;
        let context_limit =
            indra::context_limit::get_context_limit(provider.as_ref(), &model_config.model_name)
                .await?;
...
async fn create_successor_session(
    ...
    old_session: &indra::session::Session,
...
fn handle_agent_error(e: &anyhow::Error, is_stream_json_mode: bool) {
    ...
    if e.downcast_ref::<indra_providers::errors::ProviderError>()
        .map(|provider_error| {
            matches!(
                provider_error,
                indra_providers::errors::ProviderError::ContextLengthExceeded(_)
            )
        })
        .unwrap_or(false)
    {
...
async fn get_reasoner(
) -> Result<(Arc<dyn Provider>, indra_providers::model::ModelConfig), anyhow::Error> {
    use indra::providers::create;
    ...
    let model_config =
        indra::model_config::model_config_from_user_config(&provider, model.as_str())?;
    let extensions = indra::config::extensions::get_enabled_extensions_with_config(config);
...
fn build_switched_model_config(
    provider_name: &str,
    model_name: &str,
    current_model_config: &indra_providers::model::ModelConfig,
) -> Result<indra_providers::model::ModelConfig> {
    indra::model_config::model_config_from_user_config(provider_name, model_name)
        .map(|config| {
            config
...
mod tests {
    use super::*;
    use indra::agents::extension::Envs;
    use indra::config::ExtensionConfig;
    use serde_json::json;
    ...
    fn planner_history_excludes_turn_context_events() {
        use indra::conversation::message::MessageMetadata;
    ...
    fn test_build_switched_model_config_rebuilds_target_model_settings() {
        ...
        let current_model_config = indra_providers::model::ModelConfig {
        ...
        let expected = indra_providers::model::ModelConfig::new("gpt-5.4")
    ...
    fn test_build_switched_model_config_detects_effort_suffix_change() {
        ...
        let current = indra_providers::model::ModelConfig::new("gpt-5.4-high")
            .with_canonical_limits("openai");
        assert_eq!(current.model_name, "gpt-5.4");
        assert_eq!(
            current.thinking_effort(),
            Some(indra_providers::thinking::ThinkingEffort::High)
        );
    ...
    fn test_split_command_args_windows_paths() {
        assert_eq!(
            indra::utils::split_command_args(r"C:\tools\mcp.exe --arg value").unwrap(),
            vec![r"C:\tools\mcp.exe", "--arg", "value"]
        );
        assert_eq!(
            indra::utils::split_command_args(r#""C:\Program Files\server\mcp.exe" --arg"#).unwrap(),
    ...
    fn test_split_command_args_unmatched_quote() {
        assert!(indra::utils::split_command_args(r#""unmatched"#).is_err());
    }
    ...
    async fn new_session_inherits_provider_model_and_working_dir() {
        ...
        let old = sm
            .create_session(
                ...
                "CLI Session".to_string(),
                indra::session::SessionType::User,
                GooseMode::Auto,
            )
            .await
            .unwrap();
        ...
        sm.update(&old.id)
            ...
            .model_config(indra_providers::model::ModelConfig::new("test-model"))
            .accumulated_usage(indra_providers::conversation::token_usage::Usage::new(
                Some(100),
                Some(50),
                Some(150),
            ))
        ...
        let mut extension_data = indra::session::ExtensionData::new();
    ...
    impl Provider for StubProvider {
        ...
        async fn stream(
            &self,
            _model_config: &indra_providers::model::ModelConfig,
            _system: &str,
            _messages: &[Message],
            _tools: &[rmcp::model::Tool],
        ) -> std::result::Result<
            indra::providers::base::MessageStream,
            indra_providers::errors::ProviderError,
        > {
            Ok(indra::providers::base::stream_from_single_message(
                Message::assistant().with_text("stub reply"),
                ProviderUsage::new(
                    "stub".to_string(),
                    indra_providers::conversation::token_usage::Usage::default(),
                ),
            ))
        }
    }
    ...
    async fn session_with_loader(
        ...
        let agent = indra::agents::Agent::with_config(indra::agents::AgentConfig::new(
            Arc::new(session_manager),
            Arc::new(indra::config::PermissionManager::new(
                temp_dir.path().to_path_buf(),
            )),
            None,
            GooseMode::default(),
            ...
            true,
            indra::agents::GoosePlatform::GooseCli,
        ));
        agent
            .update_provider(
                Arc::new(StubProvider),
                indra_providers::model::ModelConfig::new("stub-model"),
                &session.id,
            )
            .await
            .unwrap();
