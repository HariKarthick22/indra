use anyhow::Result;
use indra::providers::utils::init_goose_request_log;
...
pub fn setup_logging(name: Option<&str>) -> &'static Result<()> {
    INIT.get_or_init(|| {
        ...
        let config = indra::logging::LoggingConfig {
            component: "cli",
            name,
        ...
        let subscriber = indra::logging::build_logging_subscriber(&config)?;
...
#[cfg(test)]
mod tests {
    use indra::tracing::langfuse_layer;
    use std::env;
    ...
    fn test_log_directory_creation() {
        ...
        let log_dir = indra::logging::prepare_log_directory("cli", true).unwrap();
    ...
    async fn test_default_filter_avoids_debug_by_default() {
        ...
        assert!(indra::logging::build_logging_subscriber(&config).is_ok());
    }
}
