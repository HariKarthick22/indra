...
use indra::agents::GoosePlatform;
use indra::builtin_extension::register_builtin_extensions;
use indra::config::{Config, GooseMode};
#[cfg(feature = "telemetry")]
use indra::posthog::get_telemetry_choice;
use indra::recipe::Recipe;
use indra::source_roots::SourceRoot;
use indra_mcp::mcp_server_runner::{serve, McpCommand};
use indra_mcp::{AutoVisualiserRouter, ComputerControllerServer, MemoryServer, TutorialServer};
...
fn parse_streamable_http_extension(input: &str) -> Result<StreamableHttpOptions, String> {
    ...
    let (mut url, mut timeout) = (String::new(), indra::config::DEFAULT_EXTENSION_TIMEOUT);
...
struct McpProbeScript {
    ...
    #[serde(default)]
    oauth: indra::oauth::OAuthFlowConfig,
...
async fn handle_mcp_probe(extension_command: String, script_path: Option<String>) -> Result<()> {
    use indra::agents::{Agent, AgentConfig, ToolCallContext};
    ...
    let script = if let Some(path) = script_path {
        ...
        McpProbeScript {
            ...
            elicitation: None,
            oauth: indra::oauth::OAuthFlowConfig::default(),
    ...
    let config = indra::config::Config::global();
    let mut agent_config = AgentConfig::new(
        std::sync::Arc::new(SessionManager::instance()),
        indra::config::permission::PermissionManager::instance(),
        None,
    ...
    let session = agent
        ...
        .create_session(
            std::env::current_dir()?,
            "MCP Probe".to_string(),
            indra::session::session_manager::SessionType::Hidden,
...
#[cfg(feature = "roaming")]
type RoamShareSlot =
    std::sync::Arc<tokio::sync::RwLock<Option<std::sync::Arc<indra_roaming::RoamingNode>>>>;

#[cfg(feature = "roaming")]
fn spawn_roam_share(
    server: std::sync::Arc<indra::acp::server_factory::AcpServer>,
) -> RoamShareSlot {
...
async fn start_roam_share(
    server: std::sync::Arc<indra::acp::server_factory::AcpServer>,
) -> Result<std::sync::Arc<indra_roaming::RoamingNode>> {
    ...
    use indra_roaming::{RoamingConfig, RoamingNode, TrustBook};
    use std::sync::Arc;
    ...
    let node = RoamingNode::bind(RoamingConfig {
        ...
        directory: indra_roaming::Directory::persistent_owned(directory_path()),
        bind_addr: None,
        relay_tls: None,
    })
    .await?;
...
async fn handle_serve_command(args: ServeCommandArgs) -> Result<()> {
    ...
    use indra::acp::server::AcpBuiltinSelection;
    use indra::acp::server_factory::{AcpServer, AcpServerFactoryConfig};
    use indra::acp::transport::create_router;
    use indra::config::paths::Paths;
    ...
    if tls {
        ...
        {
            let tls_setup = indra::acp::transport::tls::setup_tls(
    ...
    match (
        ...
        (_, _, Some(recipe_name)) => {
            ...
            let recipe_version = crate::recipes::search_recipe::load_recipe_file(recipe_name)
                .ok()
                .and_then(|rf| {
                    indra::recipe::template_recipe::parse_recipe_content(
                        &rf.content,
...
#[cfg(feature = "local-inference")]
fn print_download_progress(manager: &indra::download_manager::DownloadManager) {
    let Some(progress) = manager
        .list_progress()
        .into_iter()
        .find(|progress| progress.status == indra::download_manager::DownloadStatus::Downloading)
    else {
        return;
    };
...
fn local_search_memory_limit(ram_gb: Option<f64>) -> Result<u64> {
    ...
    match indra::providers::local_inference::InferenceRuntime::get_or_init() {
        Ok(runtime) => Ok(
            indra::providers::local_inference::available_inference_memory_bytes(runtime.as_ref()),
        ),
...
fn recommended_variant(
    model: &indra::providers::local_inference::hf_models::HfModelInfo,
    available_memory: u64,
) -> Option<&indra::providers::local_inference::hf_models::HfModelVariant> {
    use indra::providers::local_inference::hf_models::{recommend_variant, HfQuantVariant};
...
async fn handle_local_models_command(command: LocalModelsCommand) -> Result<()> {
    use indra::providers::local_inference::hf_models;

    indra::providers::local_inference::configure_huggingface_auth();
    ...
    match command {
        ...
        LocalModelsCommand::Download { spec } => {
            ...
            let manager = indra::download_manager::get_download_manager();
...
pub async fn cli() -> anyhow::Result<()> {
    register_builtin_extensions(indra_mcp::BUILTIN_EXTENSIONS.clone());
    ...
    match cli.command {
        ...
        Some(Command::Acp {
            builtins,
            enable_scheduler,
        }) => indra::acp::server::run(builtins, enable_scheduler).await,
        #[cfg(feature = "roaming")]
        ...
        Some(Command::ValidateExtensions { file }) => {
            use indra::agents::validate_extensions::validate_bundled_extensions;
