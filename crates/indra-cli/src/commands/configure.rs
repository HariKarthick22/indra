...
use console::style;
use indra::agents::extension::{ToolInfo, PLATFORM_EXTENSIONS};
use indra::agents::extension_manager::get_parameter_names;
use indra::agents::Agent;
use indra::agents::{extension::Envs, ExtensionConfig};
use indra::config::declarative_providers::{
    create_custom_provider, remove_custom_provider, AuthConfig, CreateCustomProviderParams,
};
use indra::config::extensions::{
    get_all_extension_names, get_all_extensions, get_enabled_extensions, get_extension_by_name,
    name_to_key, remove_extension, set_extension, set_extension_enabled,
};
use indra::config::paths::Paths;
use indra::config::permission::PermissionLevel;
use indra::config::signup_tetrate::TetrateAuth;
use indra::config::{
    configure_tetrate, Config, ConfigError, ExperimentManager, ExtensionEntry, GooseMode,
    PermissionManager,
};
#[cfg(feature = "telemetry")]
use indra::posthog::{get_telemetry_choice, TELEMETRY_ENABLED_KEY};
use indra::providers::base::ConfigKey;
use indra::providers::provider_test::test_provider_configuration;
use indra::providers::{create, providers, retry_operation, RetryConfig};
use indra::session::SessionType;
use indra_providers::thinking::ThinkingEffort;
...
fn interactive_model_search(
    models: &[String],
    provider_meta: &indra::providers::base::ProviderMetadata,
...
pub async fn configure_provider_dialog() -> anyhow::Result<bool> {
    ...
    let models_res = retry_operation(&RetryConfig::default(), || async {
        temp_provider
            .fetch_recommended_models(indra::model_config::global_toolshim())
            .await
    })
    .await;
...

    {
        let supports_thinking = match temp_provider.fetch_model_info(&model).await {
            ...
            Err(_) => indra_providers::model::ModelConfig::new(&model).is_reasoning_model(),
        };
    ...
    {
        Ok(()) => {
            indra::config::set_active_provider(config, &provider_name, &model)?;
...
pub fn toggle_extensions_dialog() -> anyhow::Result<()> {
    for warning in indra::config::get_warnings() {
...
fn prompt_extension_timeout() -> anyhow::Result<u64> {
    Ok(
        cliclack::input("Please set the timeout for this tool (in secs):")
            .placeholder(&indra::config::DEFAULT_EXTENSION_TIMEOUT.to_string())
...
fn configure_stdio_extension() -> anyhow::Result<()> {
    ...
    let mut parts = indra::utils::split_command_args(&command_str)?;
...
pub async fn configure_tool_permissions_dialog() -> anyhow::Result<()> {
    ...
    let model_config = indra::model_config::model_config_from_user_config(&provider_name, &model)?;
...
pub async fn handle_openrouter_auth() -> anyhow::Result<()> {
    use indra::config::{configure_openrouter, signup_openrouter::OpenRouterAuth};
    ...
    let model_config =
        match indra::model_config::model_config_from_user_config("openrouter", &configured_model) {
            Ok(config) => config,
            Err(e) => {
                    ...
                    if !has_developer {
                        set_extension(ExtensionEntry {
                            ...
                            config: ExtensionConfig::Platform {
                                ...
                                display_name: Some(indra::config::DEFAULT_DISPLAY_NAME.to_string()),
                                bundled: Some(true),
                                available_tools: Vec::new(),
                            },
                        });
...
pub async fn handle_tetrate_auth() -> anyhow::Result<()> {
    ...
    println!("\nTesting configuration...");
    let configured_model: String = config.get_goose_model()?;
    if let Err(e) = indra::model_config::model_config_from_user_config("tetrate", &configured_model)
    {
        eprintln!("⚠️  Invalid model configuration: {}", e);
        eprintln!("Your settings have been saved. Please check your model configuration.");
        return Ok(());
...
fn add_provider() -> anyhow::Result<()> {
    ...
    let models: Vec<indra_providers::base::ModelInfo> = models_input
        .split(',')
        .map(str::trim)
        .filter(|name| !name.is_empty())
        .map(indra_providers::base::ModelInfo::new)
        .collect();
...
async fn remove_provider() -> anyhow::Result<()> {
    let custom_providers_dir = indra::config::declarative_providers::custom_providers_dir();
    let custom_providers = if custom_providers_dir.exists() {
        indra::config::declarative_providers::load_custom_providers(&custom_providers_dir)?
    } else {
        Vec::new()
    };
    ...
    if let Err(e) = indra::providers::cleanup_provider(selected_id).await {
