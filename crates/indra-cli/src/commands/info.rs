...
use indra::session::session_manager::{DB_NAME, SESSIONS_FOLDER};
...
async fn check_provider(
    ...
    let model_config = indra::model_config::model_config_from_user_config(&provider, &model)
        .map_err(|e| ProviderCheckError::InvalidModel(e.to_string()))?;

    let provider_client = indra::providers::create(&provider, Vec::new())
        .await
        .map_err(|e| {
    ...
    indra::session_context::with_session_id(
