...
use indra::config::{Config, ConfigError};
use indra_roaming::{
...
const CARD_SCHEME: &str = "indra+roam://";
...
async fn handle_pair(name: Option<String>) -> Result<()> {
    ...
    indra_roaming::PeerBook::update(peerbook_path(), |book| {
...
async fn handle_peers(command: PeersCommand) -> Result<()> {
    let book = indra_roaming::PeerBook::load(peerbook_path())?;
    match command {
        ...
        PeersCommand::Remove { name } => {
            let existed =
                indra_roaming::PeerBook::update(peerbook_path(), |book| book.remove(&name))?;
            if existed {
        ...
        PeersCommand::Rename { from, to } => {
            indra_roaming::PeerBook::update(peerbook_path(), |book| book.rename(&from, &to))?;
...
fn resolve_key(book: &indra_roaming::PeerBook, target: &str) -> Result<EndpointId> {
...
async fn handle_list() -> Result<()> {
    ...
    for e in entries {
        ...
        let dir = match e.direction {
            indra_roaming::Direction::Inbound => "inbound",
            indra_roaming::Direction::Outbound => "outbound",
        };
...
async fn dial_target(
    ...
    indra_roaming::RoamingClientStream,
)> {
...
async fn handle_bridge(
    ...
    // outlive the splice.
    let indra_roaming::RoamingClientStream {
        conn,
        send: remote_send,
        recv: remote_recv,
        ..
