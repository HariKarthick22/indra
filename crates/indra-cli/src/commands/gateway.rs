use anyhow::Result;
use indra::execution::manager::AgentManager;
use indra::gateway::manager::GatewayManager;
use std::sync::Arc;
...
pub async fn handle_gateway_start(
    ...
    let mut config = indra::gateway::GatewayConfig {
        gateway_type,
        platform_config,
        max_sessions: 0,
    };

    let gw = indra::gateway::create_gateway(&mut config)?;
