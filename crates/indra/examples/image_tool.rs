...
async fn main() -> Result<()> {
    ...
    // Create providers
    let providers: Vec<(
        Arc<dyn indra::providers::base::Provider>,
        indra_providers::model::ModelConfig,
    )> = vec![
        (
            create_with_named_model("databricks", Vec::new()).await?,
            indra::model_config::model_config_from_user_config(
                "databricks",
        ...
        (
            ...
            indra::model_config::model_config_from_user_config("openai", OPEN_AI_DEFAULT_MODEL)?,
        ),
        (
