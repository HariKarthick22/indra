...
use indra::conversation::message::{Message, MessageMetadata};
use indra::custom_requests::{
    GetSessionInfoRequest, GetSessionInfoResponse, UpdateSessionProjectRequest,
};
use indra::recipe::{Recipe, Settings};
use indra::recipe_deeplink;
...
fn test_update_session_project_rejects_unknown_persisted_session_type() {
    run_test(async {
        ...
        let db_path = data_root
            .path()
            .join(indra::session::session_manager::SESSIONS_FOLDER)
            .join(indra::session::session_manager::DB_NAME);
