...
use indra::conversation::message::{Message, MessageContent, ToolRequest};
use indra::security::adversary_inspector::AdversaryInspector;
use indra::tool_inspection::ToolInspector;
...
async fn test_adversary_disabled_without_config_file() {
    ...
    let inspector = AdversaryInspector::with_config_dir(
        provider,
        Arc::new(indra::session::SessionManager::new(
...
async fn test_adversary_enabled_default_tools() {
    ...
    assert!(matches!(
        results[0].action,
        indra::tool_inspection::InspectionAction::Allow
    ));
