...
use indra::providers::base::{MessageStream, Provider};
use indra::providers::inventory::ProviderInventoryService;
use indra::session::session_manager::SessionStorage;
use indra_providers::errors::ProviderError;
use indra_providers::model::ModelConfig;
use indra_test_support::EnforceSessionId;
...
impl Provider for MockProvider {
    ...
    async fn stream(
        ...
        _system: &str,
        _messages: &[indra::conversation::message::Message],
...
fn mock_provider_factory() -> indra::acp::server::AcpProviderFactory {
    Arc::new(
...
fn write_config(config_dir: &std::path::Path) {
    ...
    std::fs::write(
        config_dir.join(indra::config::base::CONFIG_YAML_NAME),
