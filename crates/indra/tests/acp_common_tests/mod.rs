...
use fs_err as fs;
use indra::acp::server::AcpProviderFactory;
use indra::config::base::CONFIG_YAML_NAME;
use indra::config::GooseMode;
use indra::session::{EnabledExtensionsState, SessionManager};
use indra_test_support::{McpFixture, FAKE_CODE, TEST_IMAGE_B64, TEST_MODEL};
...
pub async fn run_new_session_uses_current_config_mode<C: Connection>() {
    let temp_dir = tempfile::tempdir().unwrap();
    let config_path = temp_dir.path().join(indra::config::base::CONFIG_YAML_NAME);
    fs::write(
        &config_path,
    ...
    let global_config_path =
        indra::config::paths::Paths::config_dir().join(indra::config::base::CONFIG_YAML_NAME);
    fs::write(
        &global_config_path,
        format!("GOOSE_MODEL: {TEST_MODEL}\nGOOSE_PROVIDER: openai\nGOOSE_MODE: auto\n"),
    )
