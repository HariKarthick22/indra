...
use indra_test_support::{EnforceSessionId, IgnoreSessionId, McpFixture, FAKE_CODE};
...
fn write_acp_global_config(contents: &str) -> PathBuf {
    ...
    let config_dir = indra::config::paths::Paths::config_dir();
...
fn test_custom_get_extensions() {
    ...
    run_test(async move {
        ...
        let stored_inline_token = indra::config::Config::global()
