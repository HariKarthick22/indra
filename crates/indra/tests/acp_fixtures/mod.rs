...
use fs_err as fs;
use indra::acp::server::{serve, AcpProviderFactory, GooseAcpAgent, GooseAcpAgentOptions};
pub use indra::acp::{map_permission_response, PermissionDecision};
use indra::agents::GoosePlatform;
use indra::builtin_extension::register_builtin_extensions;
use indra::config::paths::Paths;
use indra::config::{GooseMode, PermissionManager};
use indra::providers::api_client::{ApiClient, AuthMethod as ApiAuthMethod};
use indra::providers::base::Provider;
use indra::providers::openai::OpenAiProvider;
use indra::scheduler::{ScheduledJob, SchedulerError, ValidatedScheduleRecipe};
use indra::scheduler_trait::SchedulerTrait;
use indra::session::Session as GooseSession;
use indra::session_context::SESSION_ID_HEADER;
use indra_test_support::{ExpectedSessionId, TEST_MODEL};
...
fn write_global_test_config(config_path: &Path, openai_base_url: &str) {
    ...
    let global_config_path = global_config_dir.join(indra::config::base::CONFIG_YAML_NAME);
...
pub async fn spawn_acp_server_in_process(
    ...
    let config_path = data_root.join(indra::config::base::CONFIG_YAML_NAME);
    ...
    let agent = GooseAcpAgent::new(GooseAcpAgentOptions {
        provider_factory,
        builtin_selection: indra::acp::server::AcpBuiltinSelection {
