...
use indra::acp::ACP_CURRENT_MODEL;
use indra::agents::{Agent, AgentConfig, AgentEvent, GoosePlatform, PromptManager, SessionConfig};
use indra::config::{ExtensionConfig, GooseMode, PermissionManager};
use indra::conversation::message::{ActionRequiredData, Message, MessageContent};
use indra::permission::Permission;
use indra::providers::anthropic::ANTHROPIC_DEFAULT_MODEL;
use indra::providers::azure::AZURE_DEFAULT_MODEL;
use indra::providers::base::Provider;
#[cfg(feature = "aws-providers")]
use indra::providers::bedrock::BEDROCK_DEFAULT_MODEL;
use indra::providers::claude_code::CLAUDE_CODE_DEFAULT_MODEL;
use indra::providers::codex::CODEX_DEFAULT_MODEL;
use indra::providers::create_with_named_model;
use indra::providers::google::GOOGLE_DEFAULT_MODEL;
use indra::providers::litellm::LITELLM_DEFAULT_MODEL;
use indra::providers::openai::OPEN_AI_DEFAULT_MODEL;
#[cfg(feature = "aws-providers")]
use indra::providers::sagemaker_tgi::SAGEMAKER_TGI_DEFAULT_MODEL;
use indra::providers::snowflake::SNOWFLAKE_DEFAULT_MODEL;
use indra::providers::xai::XAI_DEFAULT_MODEL;
use indra::session::{SessionManager, SessionType};
use indra_providers::databricks::DATABRICKS_DEFAULT_MODEL;
use indra_providers::errors::ProviderError;
use indra_providers::thinking::ThinkingEffortSupport;
use indra_test_support::{
...
impl ProviderFixture {
    async fn setup(config: &ProviderTestConfig, mode: GooseMode) -> Result<Self> {
        ...
        let model_config = indra::model_config::model_config_from_user_config(
            &config.name.to_lowercase(),
            config.model_name,
        )?;
    ...
    async fn tool_roundtrip(
        &self,
        prompt: &str,
        model_config: Option<indra_providers::model::ModelConfig>,
    ) -> Result<Message> {
        let tools = self
            .agent
            .extension_manager
        ...
        let (response1, _) = indra::session_context::with_session_id(
        ...
        let ctx = indra::agents::ToolCallContext::new(
        ...
        let (response2, _) = indra::session_context::with_session_id(
            Some(self.session_id.clone()),
            self.provider.complete(
                &model_config,
                &system,
    ...
    async fn test_basic_response(&self) -> Result<()> {
        ...
        let (response, _) = indra::session_context::with_session_id(
            Some(self.session_id.clone()),
            self.provider.complete(
                &model_config,
    ...
    async fn test_context_length_exceeded_error(&self) -> Result<()> {
        ...
        let result = indra::session_context::with_session_id(
            Some(self.session_id.clone()),
            self.provider.complete(
                &model_config,
                "You are a helpful assistant.",
    ...
    async fn test_image_content_support(&self) -> Result<()> {
        let image_config = self.image_model.as_ref().map(|model| {
            indra_providers::model::ModelConfig::new(model).with_canonical_limits(&self.name)
        });
        let response = self
            .tool_roundtrip(
    ...
    async fn test_model_switch(&self) -> Result<()> {
        ...
        let alt_config =
            indra_providers::model::ModelConfig::new(alt).with_canonical_limits(&self.name);
