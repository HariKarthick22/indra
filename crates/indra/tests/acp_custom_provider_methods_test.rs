...
use indra::config::declarative_providers::load_provider;
use indra::config::paths::Paths;
use indra::config::{Config, ConfigError, DeclarativeProviderConfig};
...
fn acp_catalog_and_custom_provider_methods_use_core_provider_store() {
    ...
    run_test(async move {
        ...
        for valid_id in ["custom_openai", "openai-compat", "a1"] {
            assert!(
                indra::config::declarative_providers::validate_provider_id(valid_id).is_ok(),
