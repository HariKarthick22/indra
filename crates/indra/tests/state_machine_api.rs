...
use indra::agents::state_machine::{
...
use indra::session::session_manager::token_state_from_session_and_totals;
use indra::session::{Session, SessionManager, SessionType};
...
async fn custom_pipeline_supports_step_apply_run_and_usage() -> Result<()> {
    let _env = indra_test_support::otel::clear_otel_env(&[(
    ...
    assert!(matches!(
        ...
        Some(GooseEffect::Conversation(
            indra::agents::state_machine::ConversationEffect::AppendMessage(message)
