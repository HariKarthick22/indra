...
use indra_sdk_types::custom_requests::{SourceEntry, SourceType};
...
const GOOSE_DOCS_ROOT_PLACEHOLDER: &str = "{{INDRA_DOCS_ROOT}}";

/// Substitute the `{{INDRA_DOCS_ROOT}}` placeholder in the builtin
/// `indra-doc-guide` skill with the resolved docs root. INDRA ships no
/// canonical hosted docs site, so an unset `INDRA_DOCS_ROOT` resolves to
/// empty rather than falling back to any other project's documentation -
/// a different agent's docs describe different config syntax and would
/// produce wrong output if silently substituted in.
fn resolve_docs_root_placeholder(skill: &SourceEntry, content: &str, docs_root: &str) -> String {
    if skill.name != "indra-doc-guide" || skill.source_type != SourceType::BuiltinSkill {
...
fn loaded_skill_context(skill: &SourceEntry, content: &str) -> Result<String> {
    let docs_root = Config::global().get_goose_docs_root()?.unwrap_or_default();
...
mod tests {
    ...
    fn builtin_goose_doc_guide_skill() -> SourceEntry {
        SourceEntry {
            source_type: SourceType::BuiltinSkill,
            name: "indra-doc-guide".to_string(),
            description: "Test docs skill".to_string(),
            content: "Docs root: {{INDRA_DOCS_ROOT}}.".to_string(),
            path: "builtin://skills/indra-doc-guide".to_string(),
            global: true,
            writable: true,
            supporting_files: Vec::new(),
