...
use indra_providers::base::ModelInfo;
use std::str::FromStr;
...
fn is_local_provider_id(id: &str) -> bool {
    matches!(
        id.to_lowercase().as_str(),
        "ollama" | "local" | "local_inference" | "local-inference" | "openai"
...
impl GooseAcpAgent {
    pub(super) async fn on_list_providers(
        ...
        let entries = self
            ...
            .await
            .internal_err()?;
        let mut entries: Vec<_> = entries.into_iter().map(inventory_entry_to_dto).collect();
        entries.retain(|p| is_local_provider_id(&p.provider_id));
        Ok(ListProvidersResponse { entries })
    }
    ...
    pub(super) async fn on_list_provider_supported_models(
        ...
        let models = match provider.fetch_supported_models().await {
            Ok(models) => models,
            Err(indra_providers::errors::ProviderError::Authentication(error)) => {
                return Err(agent_client_protocol::Error::auth_required().data(error));
            }
            Err(indra_providers::errors::ProviderError::NotConfigured) => {
                return Err(agent_client_protocol::Error::invalid_params()
    ...
    pub(super) async fn on_list_provider_catalog(
        ...
        providers.retain(|p| is_local_provider_id(&p.provider_id));
    ...
    pub(super) async fn on_list_provider_setup_catalog(
        ...
        let mut providers: Vec<_> = crate::providers::catalog::get_setup_catalog_entries()
            .await
            .into_iter()
