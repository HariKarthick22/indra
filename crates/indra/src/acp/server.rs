...
fn resolve_default_provider_model_config(
    config: &Config,
) -> Result<(String, indra_providers::model::ModelConfig), agent_client_protocol::Error> {
...
async fn resolve_provider_default_model_config(
    provider_name: &str,
) -> Result<indra_providers::model::ModelConfig, agent_client_protocol::Error> {
...
mod tests {
    ...
    use indra_providers::conversation::token_usage::Usage as TokenUsage;
    use indra_providers::thinking::{
...
```",
    ...
    async fn asynchronous_provider_effort_update_is_forwarded_to_client() {
        ...
        session_agent
            .update_provider(
                provider.clone(),
                indra_providers::model::ModelConfig::new("gpt-4o").with_merged_request_params(
