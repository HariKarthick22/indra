...
use indra_providers::cache_semantics::apply_chat_payload_breakpoints;
...
use indra_providers::request_log::{start_log, LoggerHandleExt};
use rmcp::model::Tool;
...
impl indra_providers::base::ProviderDescriptor for LiteLLMProvider {
...
impl Provider for LiteLLMProvider {
    ...
    async fn get_context_limit(&self, model: &str, override_limit: Option<usize>) -> usize {
        indra_providers::context_limit::ContextLimitResolver::new(&self.name)
    ...
    async fn stream(
        ...
        let mut payload = indra_providers::formats::openai::create_request(
            model_config,
            system,
            messages,
            tools,
        ...
        let message = indra_providers::formats::openai::response_to_message(&response)?;
        let usage = indra_providers::formats::openai::get_usage(&response);
...
mod tests {
    ...
    use indra_providers::request_log::{install_logger, RequestLogHandle, RequestLogger};
    use serde_json::json;
    ...
    fn streaming_config_defaults_to_true() {
        let metadata = <LiteLLMProvider as indra_providers::base::ProviderDescriptor>::metadata();
        let key = metadata
            .config_keys
            .iter()
