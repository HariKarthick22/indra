...
fn deserialize_session_model_config(
    ...
    if provider_name == Some(indra_providers::azure_foundry::AZURE_FOUNDRY_PROVIDER_NAME) {
        #[derive(Deserialize)]
...
mod tests {
    ...
    use indra_providers::conversation::token_usage::{CostSource, ProviderUsage};
    ...
    fn azure_session_model_config_preserves_suffixed_deployment_id() {
        ...
        let config = deserialize_session_model_config(
            Some(indra_providers::azure_foundry::AZURE_FOUNDRY_PROVIDER_NAME),
            &json,
        )
        .unwrap();
    ...
    fn azure_session_model_config_preserves_explicit_thinking_effort() {
        ...
        assert_eq!(
            ...
            Some(indra_providers::thinking::ThinkingEffort::Low)
        );
    }
    ...
    fn non_azure_session_model_config_keeps_suffix_normalization() {
        let config = deserialize_session_model_config(
            Some(indra_providers::openai::OPEN_AI_PROVIDER_NAME),
    ...
    impl Provider for NamingTestProvider {
        ...
        ) -> std::result::Result<MessageStream, indra_providers::errors::ProviderError> {
