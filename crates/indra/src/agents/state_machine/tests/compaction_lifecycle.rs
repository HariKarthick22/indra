use anyhow::Result;
use indra_providers::conversation::token_usage::{ProviderUsage, Usage as ProviderTokenUsage};
...
async fn tokenless_provider_compacts_estimated_context() -> Result<()> {
    ...
    let pipeline = pipeline
        .with_model_config(
            indra_providers::model::ModelConfig::new("gpt-4.1").with_context_limit(Some(200)),
        )
        .await;
