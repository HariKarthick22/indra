...
use indra_agent::machine::{EffectHandler, EffectUsage, MachineSession, SessionLoader};
use indra_agent::operation::{ConversationEffect, Emitter, MachineEffect};
...
impl EffectUsage<GooseEffect> for SessionManager {
    fn usage(
        &self,
        effect: &GooseEffect,
    ) -> Option<indra_providers::conversation::token_usage::Usage> {
        match effect {
            GooseEffect::RecordUsage(usage)
            | GooseEffect::ReplaceConversation {
                usage: Some(usage), ..
...
pub(crate) async fn run(
    ...
    let mut turn_usage = indra_providers::conversation::token_usage::Usage::default();
    loop {
        let session = runtime.load(session_id).await?;
        let Some(mut result) = machine.step(&session, emit).await? else {
            break;
        };
        tracing::debug!(target: "indra::state_machine", step = result.applied_step, "applied step");
