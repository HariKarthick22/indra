...
use indra_agent::inference::InferenceEffect;
pub use indra_agent::inference::InferenceRunner;
use indra_providers::base::{MessageStream, ModelInfo, Provider};
...
fn enrich_unclaimed_tool_errors(messages: &[Message], tools: &[rmcp::model::Tool]) -> Vec<Message> {
    ...
    for message in &mut messages {
        for content in &mut message.content {
            let indra_providers::conversation::message::MessageContent::ToolResponse(response) =
                content
            else {
                continue;
            };
...
fn canonicalize_tool_request_names(
    ...
    for content in &mut message.content {
        let indra_providers::conversation::message::MessageContent::ToolRequest(request) = content
        else {
            continue;
        };
                ...
                let message = message.map(|mut message| {
                    ...
                    if message.role == rmcp::model::Role::Assistant {
                        ...
                        message.metadata.set_operation_note(
                            "specialist",
                            "specialist_name",
                            serde_json::json!(specialist_name),
                        );
                        message.metadata.set_operation_note(
                            "specialist",
                            "operation",
