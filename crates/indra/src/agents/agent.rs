...
use crate::agents::state_machine::{
    ...
    SlashCommandOperation, SpecialistOperation, StateMachine, StatusOperation, SteerOperation,
...
use indra_providers::thinking::{ThinkingEffort, ThinkingEffortSupport};
use regex::Regex;
use rmcp::model::{
...
fn normalize_legacy_provider_thinking_effort(
    mut model_config: indra_providers::model::ModelConfig,
    effort_support: &ThinkingEffortSupport,
) -> indra_providers::model::ModelConfig {
...
pub struct ReplyContext {
    ...
    pub model_config: indra_providers::model::ModelConfig,
}
...
impl Agent {
        ...
        let tool_call_cut_off = match Config::global().get_param::<usize>("GOOSE_TOOL_CALL_CUTOFF")
        {
            ...
            Err(_) => {
                let context_limit = match self.provider().await {
                    ...
                    .await
                    .unwrap_or(indra_providers::model::DEFAULT_CONTEXT_LIMIT),
                    Err(_) => indra_providers::model::DEFAULT_CONTEXT_LIMIT,
                };
    ...
    pub(super) fn create_state_machine(
        ...
        let remaining_operations: Vec<Arc<dyn Operation<Session, GooseEffect> + '_>> = vec![
            ...
            Arc::new(SpecialistOperation::new()),
    ...
    async fn reply_internal(
        ...
        let specialist_registry = crate::agents::specialists::SpecialistRegistry::new();
        let user_prompt = conversation
            ...
            .find_map(|m| {
                if m.role == rmcp::model::Role::User {
                    let text = m.user_visible_content().as_concat_text();
                    if !text.is_empty() {
                        return Some(text);
                    }
                }
            ...
            .unwrap_or_default();
        let (specialist_op, specialist) = specialist_registry.route(&user_prompt);

        if specialist.is_stub() {
            if let Some(stub_msg) = specialist.stub_message() {
                let mut stub_response = Message::assistant().with_text(stub_msg);
                stub_response.metadata.set_operation_note(
                    "specialist",
                    "specialist_name",
                    serde_json::json!(specialist.name()),
                );
                stub_response.metadata.set_operation_note(
                    "specialist",
                    "operation",
                ...
                self.config
                    .session_manager
                    .add_message(&session.id, &stub_response)
                    .await?;
                return Ok(
                    futures::stream::iter(vec![Ok(AgentEvent::Message(stub_response))]).boxed(),
                );
            }
        }
...
mod tests {
    ...
    async fn capture_tool_dispatch_fields(
        ...
        use indra_test_support::otel::clear_otel_env;
        use rmcp::object;
    ...
    impl crate::providers::base::Provider for ActionRequiredProvider {
        ...
        async fn stream(
            &self,
            _: &indra_providers::model::ModelConfig,
            _: &str,
        ...
        fn thinking_effort_support(&self) -> ThinkingEffortSupport {
            if self.applies_effort {
                ThinkingEffortSupport::Options(
                    indra_providers::thinking::ThinkingEffortCapability {
                        option_id: "effort".to_string(),
                        values: vec![indra_providers::thinking::ThinkingEffortOption {
    ...
    async fn effort_test_agent(
        ...
        agent
            .update_provider(
                provider.clone(),
                indra_providers::model::ModelConfig::new("mock-model"),
                &session.id,
            )
            .await
            .unwrap();
    ...
    async fn provider_toolshim_is_effective_without_being_persisted() {
        ...
        agent
            .update_provider(
                provider,
                indra_providers::model::ModelConfig::new("test-model"),
                &session.id,
            )
            .await
            .unwrap();
    ...
    async fn update_provider_replaces_harness_only_effort_for_legacy_provider() {
        ...
        let model_config =
            indra_providers::model::ModelConfig::new("mock-model").with_merged_request_params(
