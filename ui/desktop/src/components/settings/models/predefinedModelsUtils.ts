...
export function getPredefinedModelsFromEnv(): Model[] {
  try {
    const envModels = window.appConfig.get('INDRA_PREDEFINED_MODELS'); // process.env.INDRA_PREDEFINED_MODELS
    if (envModels && typeof envModels === 'string') {
      return JSON.parse(envModels) as Model[];
    }
  } catch (error) {
    console.warn('Failed to parse INDRA_PREDEFINED_MODELS environment variable:', error);
  }
  return [];
}
