...
function ProviderCards({
  ...
  const isLocalProvider = useCallback((provider: ProviderDetails) => {
  ...
  const providerCards = useMemo(() => {
    ...
    const localProviders = providersArray
      ...
      .map((provider) => {
        if (provider.name === 'openai') {
          return {
            ...
            metadata: {
              ...provider.metadata,
              display_name: 'Local Endpoint (vLLM / LM Studio)',
              description: 'Local OpenAI-compatible endpoint (vLLM, LM Studio, llama-server)',
            },
          };
        }
        if (provider.name === 'ollama') {
          return {
            ...
            metadata: {
              ...
              description: 'Local Ollama instance running on localhost:11434',
            },
          };
        }
        if (provider.name === 'local' || provider.name === 'local_inference' || provider.name === 'local-inference') {
          return {
            ...provider,
            metadata: {
              ...provider.metadata,
              display_name: 'Local Inference (MLX / GGUF)',
              description: 'Embedded offline model execution via MLX and llama.cpp',
            },
          };
        }
        return provider;
    ...
    const sortedProviders = [...localProviders].sort(
      (a, b) =>
