...
export const ModeSection = () => {
  ...
  const handleModeChange = async (newMode: string) => {
    try {
      await upsert('INDRA_MODE', newMode, false);
  ...
  useEffect(() => {
    const mode = config.INDRA_MODE as string | undefined;
    if (mode) {
      setCurrentMode(mode);
    }
  }, [config.INDRA_MODE]);
  ...
  const fetchMaxTurns = useCallback(async () => {
    try {
      const turns = (await read('INDRA_MAX_TURNS', false)) as number;
      if (turns) {
        setMaxTurns(turns);
      }
    } catch (error) {
  ...
  const handleMaxTurnsChange = async (value: number) => {
    try {
      await upsert('INDRA_MAX_TURNS', value, false);
      setMaxTurns(value);
    } catch (error) {
