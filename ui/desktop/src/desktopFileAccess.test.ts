...
describe('DesktopFileAccess', () => {
  it('reads .indrahints from the bound working directory', async () => {
    const workingDirectory = makeTempDirectory();
    fs.writeFileSync(path.join(workingDirectory, '.indrahints'), 'project guidance');
    ...
    await expect(access.readGoosehints(7)).resolves.toEqual({
      ...
      filePath: path.join(canonicalWorkingDirectory, '.indrahints'),
      error: null,
      found: true,
    });
  });
  ...
  it('creates and updates .indrahints in the bound working directory', async () => {
    const workingDirectory = makeTempDirectory();
    const access = new DesktopFileAccess();
    await access.bindWindow(7, workingDirectory);
    const filePath = path.join(fs.realpathSync(workingDirectory), '.indrahints');
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      fs.writeFileSync(path.join(workingDirectory, '.indrahints'), 'original guidance');
      fs.writeFileSync(path.join(replacementDirectory, '.indrahints'), 'replacement guidance');
      const access = new DesktopFileAccess();
      await access.bindWindow(7, workingDirectory);
      ...
      expect(fs.readFileSync(path.join(originalDirectory, '.indrahints'), 'utf8')).toBe(
        'original guidance'
      );
      expect(fs.readFileSync(path.join(replacementDirectory, '.indrahints'), 'utf8')).toBe(
  ...
  it('rejects a canonical working directory replaced by another directory', async () => {
    ...
    fs.renameSync(workingDirectory, originalDirectory);
    fs.mkdirSync(workingDirectory);
    fs.writeFileSync(path.join(workingDirectory, '.indrahints'), 'replacement guidance');
    ...
    expect(fs.readFileSync(path.join(workingDirectory, '.indrahints'), 'utf8')).toBe(
      'replacement guidance'
    );
  });
  ...
  it('rejects a bound working directory that was renamed away', async () => {
    ...
    expect(result.found).toBe(false);
    expect(result.error).toContain('working directory changed');
    expect(fs.readFileSync(path.join(renamedDirectory, '.indrahints'), 'utf8')).toBe(
      'original guidance'
    );
  });

  it.skipIf(process.platform === 'win32')(
    'rechecks the working directory before truncating an opened .indrahints',
    async () => {
      const root = makeTempDirectory();
      const workingDirectory = path.join(root, 'project');
      const renamedDirectory = path.join(root, 'renamed-project');
      const filePath = path.join(workingDirectory, '.indrahints');
      fs.mkdirSync(workingDirectory);
      ...
      vi.spyOn(fsPromises, 'open').mockImplementationOnce(async (...args) => {
        ...
        fs.mkdirSync(workingDirectory);
        fs.linkSync(
          path.join(renamedDirectory, '.indrahints'),
          path.join(workingDirectory, '.indrahints')
        );
        return open(...args);
      });
  ...
  it.skipIf(process.platform === 'win32')(
    'rechecks the working directory before reading an opened .indrahints',
    async () => {
      const root = makeTempDirectory();
      const workingDirectory = path.join(root, 'project');
      const renamedDirectory = path.join(root, 'renamed-project');
  ...
  it('rechecks the working directory before creating a missing .indrahints', async () => {
    const root = makeTempDirectory();
    const workingDirectory = path.join(root, 'project');
    const renamedDirectory = path.join(root, 'renamed-project');
    fs.mkdirSync(workingDirectory);
    ...
    vi.spyOn(fsPromises, 'lstat').mockImplementation(async (...args) => {
      try {
        ...
        if (path.basename(args[0].toString()) === '.indrahints') {
          fs.renameSync(workingDirectory, renamedDirectory);
          fs.mkdirSync(workingDirectory);
        }
        throw error;
    ...
    expect(fs.existsSync(path.join(workingDirectory, '.indrahints'))).toBe(false);
  });
  ...
  it.skipIf(process.platform === 'win32')(
    'blocks a .indrahints symlink that escapes the working directory',
    async () => {
      const root = makeTempDirectory();
      const workingDirectory = path.join(root, 'project');
      ...
      fs.symlinkSync('../secret', path.join(workingDirectory, '.indrahints'));
      const access = new DesktopFileAccess();
      await access.bindWindow(7, workingDirectory);
  ...
  it('does not truncate a replacement file opened after validation', async () => {
    const workingDirectory = makeTempDirectory();
    const filePath = path.join(workingDirectory, '.indrahints');
    const originalPath = path.join(workingDirectory, 'original.indrahints');
    fs.writeFileSync(filePath, 'original guidance');
    const access = new DesktopFileAccess();
    await access.bindWindow(7, workingDirectory);
    const open = fsPromises.open.bind(fsPromises);
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      fs.writeFileSync(path.join(firstProject, '.indrahints'), 'first guidance');
      fs.writeFileSync(path.join(secondProject, '.indrahints'), 'second guidance');
      ...
      await expect(access.readGoosehints(7)).resolves.toEqual({
        file: 'first guidance',
        filePath: path.join(canonicalFirstProject, '.indrahints'),
        error: null,
        found: true,
      });
      await expect(access.writeGoosehints(7, 'updated first guidance')).resolves.toBe(true);
      expect(fs.readFileSync(path.join(firstProject, '.indrahints'), 'utf8')).toBe(
        'updated first guidance'
      );
      expect(fs.readFileSync(path.join(secondProject, '.indrahints'), 'utf8')).toBe(
        'second guidance'
      );
    }
  );
  ...
  it.skipIf(process.platform === 'win32')(
    'rejects a non-regular .indrahints target without blocking',
    async () => {
      const workingDirectory = makeTempDirectory();
      execFileSync('mkfifo', [path.join(workingDirectory, '.indrahints')]);
      const access = new DesktopFileAccess();
      await access.bindWindow(7, workingDirectory);
