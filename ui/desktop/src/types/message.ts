...
export function getSpecialistInfo(message: Message): {
  name: string;
  operation?: string;
} | null {
  ...
  if (typeof meta.specialist_name === 'string' && meta.specialist_name) {
    return {
      name: meta.specialist_name,
      operation: typeof meta.operation === 'string' ? meta.operation : undefined,
    };
  }

  const ops = (meta.operations as Record<string, Record<string, unknown>> | undefined)?.specialist;
  if (ops && typeof ops.specialist_name === 'string' && ops.specialist_name) {
    return {
      name: ops.specialist_name,
      operation: typeof ops.operation === 'string' ? ops.operation : undefined,
    };
  }
