...
export function sanitizeGoosePathRoot(env: { INDRA_PATH_ROOT?: string }): string | undefined {
  const pathRoot = resolveGoosePathRoot(env.INDRA_PATH_ROOT);
  if (pathRoot) {
    env.INDRA_PATH_ROOT = pathRoot;
  } else {
    delete env.INDRA_PATH_ROOT;
  }
  return pathRoot;
}
