export const getInitialWorkingDir = (): string => {
  ...
  return (window.appConfig?.get('INDRA_WORKING_DIR') as string) ?? '';
};

/**
 * Resolve the working directory for a new chat in the current window.
 *
 * INDRA_WORKING_DIR is fixed when the window is created, so it goes stale when
 ...
 * chats in the same window. Env-mode backends (INDRA_EXTERNAL_BACKEND) always
...
export const getEffectiveWorkingDir = async (): Promise<string> => {
  ...
  const boundUrl = window.appConfig?.get('INDRA_EXTERNAL_BACKEND_URL') as string | undefined;
  const source = window.appConfig?.get('INDRA_EXTERNAL_BACKEND_SOURCE') as string | undefined;
  if (window.appConfig?.get('INDRA_EXTERNAL_BACKEND') !== true || !boundUrl) {
    return initial;
  }
  try {
