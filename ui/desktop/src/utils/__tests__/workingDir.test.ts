...
describe('getEffectiveWorkingDir', () => {
  ...
  const mockWindow = (externalBackend: boolean, boundUrl: string, source = 'settings') => {
    appConfigGetMock.mockImplementation((key: string) => {
      if (key === 'INDRA_EXTERNAL_BACKEND') return externalBackend;
      if (key === 'INDRA_EXTERNAL_BACKEND_URL') return boundUrl;
      if (key === 'INDRA_EXTERNAL_BACKEND_SOURCE') return source;
      if (key === 'INDRA_WORKING_DIR') return '/Users/johannes/home/workspace';
      return undefined;
    });
  };
