...
export function setupAutoUpdater(tray?: Tray) {
  ...
  // Respect INDRA_DISABLE_AUTO_DOWNLOAD env var (takes precedence over user setting)
  const envDisabled =
    process.env.INDRA_DISABLE_AUTO_DOWNLOAD === '1' ||
    process.env.INDRA_DISABLE_AUTO_DOWNLOAD === 'true';
  if (envDisabled) {
    autoDownloadDisabled = true;
    log.info('Auto-download disabled via INDRA_DISABLE_AUTO_DOWNLOAD environment variable');
  }
...
function updateTrayIcon(hasUpdate: boolean) {
  ...
  if (hasUpdate) {
    ...
    trayRef.setToolTip('INDRA - Update Available');
  } else {
    // Use normal icon
    if (isDev) {
