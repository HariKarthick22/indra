...
if [ -n "${INDRA_PATH_ROOT:-}" ]; then
    RESOLVED_GOOSE_CONFIG_DIR="${INDRA_PATH_ROOT}/config"
elif [ -n "${INDRA_CONFIG_DIR:-}" ]; then
    log "INDRA_CONFIG_DIR is deprecated for desktop shims; prefer INDRA_PATH_ROOT."
    RESOLVED_GOOSE_CONFIG_DIR="${INDRA_CONFIG_DIR}"
else
...
log "Checking for INDRA_NPM_REGISTRY and INDRA_NPM_CERT environment variables for custom npm registry setup..."
# Check if INDRA_NPM_REGISTRY is set and accessible
if [ -n "${INDRA_NPM_REGISTRY:-}" ] && curl -s --head --fail "${INDRA_NPM_REGISTRY}" > /dev/null; then
    log "Checking custom goose registry availability: ${INDRA_NPM_REGISTRY}"
    log "${INDRA_NPM_REGISTRY} is accessible. Using it for npm registry."
    export NPM_CONFIG_REGISTRY="${INDRA_NPM_REGISTRY}"

    # Check if INDRA_NPM_CERT is set and accessible
    if [ -n "${INDRA_NPM_CERT:-}" ] && curl -s --head --fail "${INDRA_NPM_CERT}" > /dev/null; then
        log "Downloading certificate from: ${INDRA_NPM_CERT}"
        curl -sSL -o "${MCP_HERMIT_DIR}/cert.pem" "${INDRA_NPM_CERT}"
        if [ $? -eq 0 ]; then
        ...
        log "INDRA_NPM_CERT is either not set or not accessible. Skipping certificate setup."
    fi

else
    log "INDRA_NPM_REGISTRY is either not set or not accessible. Falling back to default npm registry."
