...
const mockElectron = {
  getConfig: vi.fn().mockReturnValue({
    INDRA_ALLOWLIST_WARNING: false,
    INDRA_WORKING_DIR: '/test/dir',
  }),
  logInfo: vi.fn(),
  on: vi.fn(),
  off: vi.fn(),
...
const mockAppConfig = {
  get: vi.fn((key: string): string | null => {
    if (key === 'INDRA_WORKING_DIR') return '/test/dir';
    return null;
  }),
};
...
describe('App Component - Brand New State', () => {
  ...
  it('should redirect to "/" when app is brand new (no provider configured)', async () => {
    ...
    mockElectron.getConfig.mockReturnValue({
      INDRA_DEFAULT_PROVIDER: null,
      INDRA_DEFAULT_MODEL: null,
      INDRA_ALLOWLIST_WARNING: false,
    });
  ...
  it('should handle deep links correctly when app is brand new', async () => {
    ...
    expect(screen.getByTestId('provider-selection-heading')).toBeInTheDocument();
  });

  it('should not redirect when provider is configured', async () => {
    // Mock provider configured
    mockElectron.getConfig.mockReturnValue({
      INDRA_DEFAULT_PROVIDER: 'openai',
      INDRA_DEFAULT_MODEL: 'gpt-4',
      INDRA_ALLOWLIST_WARNING: false,
    });
