...
import { startGooseServe } from './indraServe';
import { getLoginShellPath } from './loginShellPath';
import { GooseServeLeaseRegistry, type GooseServeLease } from './indraServeLeaseRegistry';
...
const MENU_TRANSLATIONS_ZH_CN: Record<string, string> = {
  ...
  'Focus INDRA Window': '聚焦 INDRA 窗口',
  'Quick Launcher': '快速启动器',
  'Always on Top': '窗口置顶',
  'Toggle Navigation': '切换导航',
  'About INDRA': '关于 INDRA',
...
function getConfiguredGooseLocale(): string | undefined {
  ...
  if (process.env.INDRA_LOCALE) {
    return process.env.INDRA_LOCALE;
  }
...
app.whenReady().then(() => {
  appConfig.INDRA_LOCALE = getConfiguredGooseLocale();
});
...
const getBundledConfig = (): BundledConfig => {
  ...
  //{env-macro-end}//
  return {
    defaultProvider: process.env.INDRA_DEFAULT_PROVIDER,
    defaultModel: process.env.INDRA_DEFAULT_MODEL,
    predefinedModels: process.env.INDRA_PREDEFINED_MODELS,
    version: process.env.INDRA_VERSION,
  };
};
...
const getExternalBackendUrlFromEnv = (): string | null => {
  if (!process.env.INDRA_EXTERNAL_BACKEND) {
    return null;
  }

  const configuredUrl = process.env.INDRA_EXTERNAL_BACKEND_URL?.trim();
  if (configuredUrl) {
    return configuredUrl;
  }

  return `http://127.0.0.1:${process.env.INDRA_PORT || '3000'}`;
};
...
const getExternalBackendFromEnv = (): ExternalBackend | null => {
  ...
  const secret = process.env.INDRA_SERVER__SECRET_KEY;
  if (!secret) {
    throw new Error(
      'INDRA_SERVER__SECRET_KEY must be set when using INDRA_EXTERNAL_BACKEND. ' +
...
let appConfig = {
  INDRA_DEFAULT_PROVIDER: defaultProvider,
  INDRA_DEFAULT_MODEL: defaultModel,
  INDRA_PREDEFINED_MODELS: predefinedModels,
  INDRA_PATH_ROOT: sanitizeGoosePathRoot(process.env),
  INDRA_WORKING_DIR: '',
  ...
  INDRA_EXTERNAL_BACKEND: false,
  INDRA_EXTERNAL_BACKEND_URL: '',
  INDRA_EXTERNAL_BACKEND_SOURCE: '',
  ...
  INDRA_LOCALE: process.env.INDRA_LOCALE || undefined,
  // If INDRA_ALLOWLIST_WARNING env var is not set, defaults to false (strict blocking mode)
  INDRA_ALLOWLIST_WARNING: process.env.INDRA_ALLOWLIST_WARNING === 'true',
  INDRA_DISABLE_NOSTR_SHARING: process.env.INDRA_DISABLE_NOSTR_SHARING === 'true',
};
  ...
  if (externalBackend) {
    ...
    try {
      ...
      if (!externalBackendReady) {
        ...
        const response = dialog.showMessageBoxSync({
          ...
          detail:
            'The external backend must be running and the configured secret must match INDRA_SERVER__SECRET_KEY on the server.',
    ...
    try {
      gooseServeResult = await startGooseServe({
        ...
        tls: true,
        env: {
          INDRA_PATH_ROOT: appConfig.INDRA_PATH_ROOT as string | undefined,
        },
        loginShellPath,
    ...
    mainWindow = new BrowserWindow({
      ...
      webPreferences: {
        ...
        additionalArguments: [
          JSON.stringify({
            ...appConfig,
            INDRA_LOCALE: getConfiguredGooseLocale(),
            INDRA_WORKING_DIR: workingDir,
            INDRA_EXTERNAL_BACKEND: externalBackend !== null,
            INDRA_EXTERNAL_BACKEND_URL: externalBackend?.url ?? '',
            INDRA_EXTERNAL_BACKEND_SOURCE: externalBackend?.source ?? '',
            REQUEST_DIR: dir,
            INDRA_VERSION: version,
  ...
  if (currentWindow) {
    try {
      const currentWorkingDir = await currentWindow.webContents.executeJavaScript(
        `window.appConfig ? window.appConfig.get('INDRA_WORKING_DIR') : null`
      );
...
ipcMain.handle('select-recipe-file', async (event) => {
  ...
  const pathRoot = appConfig.INDRA_PATH_ROOT as string | undefined;
...
async function appMain() {
  ...
  const appMenu = menu?.items.find((item) => item.label === 'INDRA' || item.label === 'Goose');
  if (appMenu?.submenu) {
  ...
  if (menu && process.platform !== 'darwin') {
    ...
    if (helpMenu.submenu) {
      ...
      // Create the About INDRA menu item with a submenu
      const aboutGooseMenuItem = new MenuItem({
        label: menuT('About INDRA'),
  ...
  ipcMain.handle('launch-app', async (event, gooseApp: GooseApp) => {
    try {
      ...
      const launchingWorkingDir = await launchingWindow.webContents
        .executeJavaScript(`window.appConfig ? window.appConfig.get('INDRA_WORKING_DIR') : null`)
        .catch((error) => {
...
app.whenReady().then(async () => {
  try {
    ...
    dialog.showErrorBox('INDRA Error', `Failed to create main window: ${error}`);
    app.quit();
  }
});

async function getAllowList(): Promise<string[]> {
  if (!process.env.INDRA_ALLOWLIST) {
    return [];
  }

  const response = await fetch(process.env.INDRA_ALLOWLIST);
