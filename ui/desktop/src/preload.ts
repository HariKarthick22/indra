...
const electronAPI: ElectronAPI = {
  ...
  getVersion: (): string => {
    return config.INDRA_VERSION || ipcRenderer.sendSync('get-app-version') || '';
  },
  checkForUpdates: (): Promise<{ updateInfo: unknown; error: string | null }> => {
...
function getAppLocale(): unknown {
  try {
    return ipcRenderer.sendSync('get-app-locale') ?? config.INDRA_LOCALE;
  } catch {
    return config.INDRA_LOCALE;
  }
}

const appConfigAPI: AppConfigAPI = {
  get: (key: string) => (key === 'INDRA_LOCALE' ? getAppLocale() : config[key]),
  getAll: () => ({ ...config, INDRA_LOCALE: getAppLocale() }),
};
