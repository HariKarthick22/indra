...
export class DesktopFileAccess {
  ...
  private bindingForWindow(windowId: number): WorkingDirectoryBinding {
    ...
    if (!binding) {
      throw new Error('This window is not authorized to access .indrahints');
    }
    return binding;
  }
  ...
  async readGoosehints(windowId: number): Promise<FileReadResult> {
    ...
    const filePath = path.join(binding.path, '.indrahints');
    ...
    try {
      ...
      if (metadata.isSymbolicLink()) {
        return failedRead(filePath, 'Refusing to read a symbolic link as .indrahints');
      }
      if (!metadata.isFile()) {
        return failedRead(filePath, '.indrahints is not a regular file');
      }

      const canonicalFilePath = await fs.realpath(filePath);
      if (path.dirname(canonicalFilePath) !== binding.path) {
        return failedRead(filePath, '.indrahints resolves outside the working directory');
      }
      ...
      try {
        ...
        if (openedMetadata.dev !== metadata.dev || openedMetadata.ino !== metadata.ino) {
          return failedRead(filePath, '.indrahints changed while it was being opened');
        }
        if (!(await this.bindingMatchesDirectory(binding))) {
          return failedRead(filePath, 'The working directory changed after it was authorized');
        }
      ...
      return failedRead(filePath, 'Unable to read .indrahints');
    }
  }
