...
describe('getLocale', () => {
  ...
  it('respects INDRA_LOCALE over navigator.languages', () => {
    mockAppConfig({ INDRA_LOCALE: 'en' });
    vi.stubGlobal('navigator', { languages: ['xx-XX'] });
    expect(getLocale()).toEqual({ locale: 'en', messageLocale: 'en' });
  });

  it('preserves regional tag from INDRA_LOCALE', () => {
    mockAppConfig({ INDRA_LOCALE: 'en-GB' });
    vi.stubGlobal('navigator', { languages: ['xx-XX'] });
  ...
  it('supports explicit Korean locale', () => {
    mockAppConfig({ INDRA_LOCALE: 'ko' });
    vi.stubGlobal('navigator', { languages: ['xx-XX'] });
    expect(getLocale()).toEqual({ locale: 'ko', messageLocale: 'ko' });
  });

  it('supports POSIX-style Korean locale from INDRA_LOCALE', () => {
    mockAppConfig({ INDRA_LOCALE: 'ko_KR' });
    vi.stubGlobal('navigator', { languages: ['xx-XX'] });
    expect(getLocale()).toEqual({ locale: 'ko-KR', messageLocale: 'ko' });
  });
  ...
  it('supports POSIX-style Japanese locale from INDRA_LOCALE', () => {
    mockAppConfig({ INDRA_LOCALE: 'ja_JP' });
    vi.stubGlobal('navigator', { languages: ['xx-XX'] });
    expect(getLocale()).toEqual({ locale: 'ja-JP', messageLocale: 'ja' });
  });
