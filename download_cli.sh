...
#
# Usage:
#   curl -fsSL https://github.com/HariKarthick22/indra/releases/download/stable/download_cli.sh | bash
#
# Environment variables:
#   INDRA_BIN_DIR  - Directory to which goose will be installed (default: $HOME/.local/bin)
#   INDRA_VERSION  - Optional: specific version to install (e.g., "v1.0.25"). Overrides CANARY. Can be in the format vX.Y.Z, vX.Y.Z-suffix, or X.Y.Z
#   INDRA_PROVIDER - Optional: provider for goose
#   INDRA_MODEL    - Optional: model for goose
#   INDRA_LINUX_VARIANT - Optional: Linux package variant to install (`standard`, `vulkan`, or `musl`)
#   INDRA_WINDOWS_VARIANT - Optional: Windows package variant to install (`standard` or `cuda`)
...
INDRA_BIN_DIR="${INDRA_BIN_DIR:-$DEFAULT_BIN_DIR}"
RELEASE="${CANARY:-false}"
CONFIGURE="${CONFIGURE:-true}"
INDRA_LINUX_VARIANT="${INDRA_LINUX_VARIANT:-}"
INDRA_WINDOWS_VARIANT="${INDRA_WINDOWS_VARIANT:-standard}"
if [ -n "${INDRA_VERSION:-}" ]; then
  # Validate the version format
  if [[ ! "$INDRA_VERSION" =~ ^v?[0-9]+\.[0-9]+\.[0-9]+(-.*)?$ ]]; then
    echo "[error]: invalid version '$INDRA_VERSION'."
    echo "  expected: semver format vX.Y.Z, vX.Y.Z-suffix, or X.Y.Z"
    exit 1
  fi
  INDRA_VERSION=$(echo "$INDRA_VERSION" | sed 's/^v\{0,1\}/v/') # Ensure the version string is prefixed with 'v' if not already present
  RELEASE_TAG="$INDRA_VERSION"
else
  # If INDRA_VERSION is not set, fall back to existing behavior for backwards compatibility
...
if [ "$OS" = "linux" ] && [ -n "${TERMUX_VERSION:-}" ] && [ -z "$INDRA_LINUX_VARIANT" ]; then
  echo "Termux detected (v$TERMUX_VERSION). Using musl portable build."
  INDRA_LINUX_VARIANT="musl"
fi

if [ "$OS" = "linux" ] && [ -z "$INDRA_LINUX_VARIANT" ]; then
  if detect_linux_musl; then
    INDRA_LINUX_VARIANT="musl"
  else
    INDRA_LINUX_VARIANT="standard"
  fi
elif [ -z "$INDRA_LINUX_VARIANT" ]; then
  INDRA_LINUX_VARIANT="standard"
fi
...
if [ "$OS" = "darwin" ]; then
  ...
  case "$INDRA_WINDOWS_VARIANT" in
    standard|cuda) ;;
    *)
      echo "Error: Unsupported INDRA_WINDOWS_VARIANT '$INDRA_WINDOWS_VARIANT'. Expected 'standard' or 'cuda'."
      exit 1
      ;;
  esac
  ...
  if [ "$INDRA_WINDOWS_VARIANT" = "cuda" ]; then
    FILE="goose-$ARCH-pc-windows-msvc-cuda.zip"
  fi
  EXTRACT_CMD="unzip"
  OUT_FILE="goose.exe"
else
  case "$INDRA_LINUX_VARIANT" in
    standard|vulkan|musl) ;;
    *)
      echo "Error: Unsupported INDRA_LINUX_VARIANT '$INDRA_LINUX_VARIANT'. Expected 'standard', 'vulkan', or 'musl'."
      exit 1
      ;;
  esac
  FILE="goose-$ARCH-unknown-linux-gnu.tar.bz2"
  if [ "$INDRA_LINUX_VARIANT" = "vulkan" ]; then
    FILE="goose-$ARCH-unknown-linux-gnu-vulkan.tar.bz2"
  elif [ "$INDRA_LINUX_VARIANT" = "musl" ]; then
...
if ! curl -sLf "$DOWNLOAD_URL" --output "$FILE"; then
  ...
  if ! [ -n "${INDRA_VERSION:-}" ] && [ "${CANARY:-false}" != "true" ]; then
    LATEST_TAG=$(curl -s https://api.github.com/repos/HariKarthick22/indra/releases/latest | \
...
# --- 5) Install to $INDRA_BIN_DIR ---
if [ ! -d "$INDRA_BIN_DIR" ]; then
  echo "Creating directory: $INDRA_BIN_DIR"
  mkdir -p "$INDRA_BIN_DIR"
fi

echo "Moving goose to $INDRA_BIN_DIR/$OUT_FILE"
if [ "$OS" = "windows" ]; then
  mv "$EXTRACT_DIR/goose.exe" "$INDRA_BIN_DIR/$OUT_FILE"
else
  ...
  if [ -f "$INDRA_BIN_DIR/$OUT_FILE" ]; then
    mv "$INDRA_BIN_DIR/$OUT_FILE" "$INDRA_BIN_DIR/$OUT_FILE.old"
    if ! mv "$EXTRACT_DIR/goose" "$INDRA_BIN_DIR/$OUT_FILE"; then
      echo "Error: failed to install new binary, restoring previous version"
      mv "$INDRA_BIN_DIR/$OUT_FILE.old" "$INDRA_BIN_DIR/$OUT_FILE"
      exit 1
    fi
    rm -f "$INDRA_BIN_DIR/$OUT_FILE.old"
  else
    mv "$EXTRACT_DIR/goose" "$INDRA_BIN_DIR/$OUT_FILE"
  fi
fi
...
if [[ ":$PATH:" != *":$INDRA_BIN_DIR:"* ]]; then
  echo ""
  echo "Warning: goose installed, but $INDRA_BIN_DIR is not in your PATH."
  ...
  if [ "$OS" = "windows" ]; then
    ...
    echo ""
    echo "The \$INDRA_BIN_DIR is not in your PATH."
    ...
    if [ "$CONFIGURE" = true ]; then
      ...
      1)
        ...
        echo "Adding \$INDRA_BIN_DIR to $RC_FILE..."
        echo "export PATH=\"$INDRA_BIN_DIR:\$PATH\"" >> "$RC_FILE"
      ...
      2)
        ...
        echo "    export PATH=\"$INDRA_BIN_DIR:\$PATH\""
        echo "Then reload your shell (e.g. 'source ~/.${SHELL_NAME}rc') to apply changes."
        ;;
      *)
        echo "Invalid choice. Please add \$INDRA_BIN_DIR to your PATH manually."
        ;;
      esac
    else
      echo ""
      echo "Configure disabled. Please add \$INDRA_BIN_DIR to your PATH manually."
    fi
