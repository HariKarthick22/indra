...
if [[ -z "${SKIP_BUILD:-}" && -z "${INDRA_BIN:-}" ]]; then
  echo "Building goose..."
  (cd "$REPO_ROOT" && cargo build --bin indra)
  echo ""
fi
...
def get_weather(
    ...
    return f"INDRA_TOOL_CALL_OK: The weather in {location} is 68 F and clear."
EOF
...
run_model() {
  ...
  if [[ "$RUN_TIMEOUT" -eq 0 ]]; then
    INDRA_MODE=auto INDRA_PROVIDER=openrouter INDRA_MODEL="$model" \
      "$INDRA_BIN" run --no-profile --max-turns 4 --recipe recipe.yaml
    return $?
  fi
  ...
  ' \
    "$RUN_TIMEOUT" \
    env INDRA_MODE=auto INDRA_PROVIDER=openrouter INDRA_MODEL="$model" \
    "$INDRA_BIN" run --no-profile --max-turns 4 --recipe recipe.yaml
}
...
for model in "${MODELS[@]}"; do
  ...
  if (cd "$TESTDIR" && run_model "$model" 2>&1) | tee "$log_file"; then
    if error_summary=$(summarize_error "$log_file"); then
      ...
      grep -Fq "INDRA_TOOL_CALL_OK:" "$log_file"; then
