...
#
# Knobs:
#   INDRA_PROVIDER (default: anthropic)
#   INDRA_MODEL    (default: claude-haiku-4-5)
...
export INDRA_PROVIDER="${INDRA_PROVIDER:-anthropic}"
export INDRA_MODEL="${INDRA_MODEL:-claude-haiku-4-5}"

echo "Using provider: $INDRA_PROVIDER"
echo "Using model:    $INDRA_MODEL"
echo ""
...
run_goose() {
  local prompt="$1"
  local outfile="$2"
  (cd "$TESTDIR" && "$INDRA_BIN" run --text "$prompt" --no-session 2>&1) | tee "$outfile"
}
...
EOF
  ...
  local verdict
  verdict=$("$INDRA_BIN" run --text "$judge_prompt" --no-session 2>&1)
...
EMPTYDIR=$(mktemp -d)
TMP5=$(mktemp)
(cd "$EMPTYDIR" && "$INDRA_BIN" run --text "@janpier where is the treasure?" --no-session 2>&1) | tee "$TMP5"
