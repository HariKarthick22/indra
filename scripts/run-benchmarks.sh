...
function show_usage() {
  ...
  echo "  -t, --toolshim           Enable toolshim mode by setting INDRA_TOOLSHIM=1"
  echo "  -m, --toolshim-model     Set the toolshim model (sets INDRA_TOOLSHIM_MODEL)"
...
if [ "$DEBUG_MODE" = true ]; then
  if [ -f "./target/debug/goose" ]; then
    INDRA_CMD="./target/debug/goose"
    echo "Using debug binary: $INDRA_CMD"
  else
  ...
  if [ -f "./target/release/goose" ]; then
    INDRA_CMD="./target/release/goose"
    echo "Using release binary: $INDRA_CMD"
  else
...
for ((i=0; i<$COUNT; i++)); do
  ...
  export INDRA_PROVIDER="$provider"
  export INDRA_MODEL="$model"
  
  # Set toolshim environment variables if enabled
  if [ "$TOOLSHIM" = true ]; then
    export INDRA_TOOLSHIM=1
    if [[ -n "$TOOLSHIM_MODEL" ]]; then
      export INDRA_TOOLSHIM_OLLAMA_MODEL="$TOOLSHIM_MODEL"
    fi
  fi
  ...
  if $INDRA_CMD bench --suites "$SUITES" --output "$OUTPUT_FILE" --format json; then
