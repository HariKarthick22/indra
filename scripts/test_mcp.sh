...
TEST_PROVIDER=${INDRA_PROVIDER:-anthropic}
TEST_MODEL=${INDRA_MODEL:-claude-haiku-4-5-20251001}
...
TMPFILE=$(mktemp)
(cd "$TESTDIR" && INDRA_PROVIDER="$TEST_PROVIDER" INDRA_MODEL="$TEST_MODEL" \
    "$INDRA_BIN" run --recipe recipe.yaml 2>&1) | tee "$TMPFILE"
