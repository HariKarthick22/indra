...
validate_compaction() {
  ...
  local session_json=$($INDRA_BIN session export --format json --session-id "$session_id" 2>&1)
...
(cd "$TESTDIR" && "$INDRA_BIN" run --with-builtin developer --text "list files and read hello.txt" 2>&1) | tee "$OUTPUT"
...
if ! command -v jq &> /dev/null; then
  ...
  SESSION_ID=$("$INDRA_BIN" session list --format json 2>/dev/null | jq -r '.[0].id' 2>/dev/null)
  ...
  if [ -z "$SESSION_ID" ] || [ "$SESSION_ID" = "null" ]; then
    ...
    (cd "$TESTDIR" && "$INDRA_BIN" run --resume --session-id "$SESSION_ID" --text "Please compact this conversation" 2>&1) | tee -a "$OUTPUT"
...
export INDRA_AUTO_COMPACT_THRESHOLD=0.005
...
(cd "$TESTDIR" && "$INDRA_BIN" run --text "$LONG_RESPONSE_PROMPT" 2>&1) | tee "$OUTPUT"
...
if ! command -v jq &> /dev/null; then
  ...
  if [ -z "$SESSION_ID" ] || [ "$SESSION_ID" = "null" ]; then
    ...
    (cd "$TESTDIR" && "$INDRA_BIN" run --resume --session-id "$SESSION_ID" --text "hi again" 2>&1) | tee -a "$OUTPUT"
...
unset INDRA_AUTO_COMPACT_THRESHOLD
...
if ! (cd "$PROXY_DIR" && uv sync 2>&1 | tee "$PROXY_SETUP_LOG"); then
  ...
  if [ "$PROXY_READY" != "true" ]; then
    ...
    export INDRA_PROVIDER_SKIP_BACKOFF=true
    export INDRA_PROVIDER=anthropic
    export INDRA_MODEL=claude-haiku-4-5
    ...
    export INDRA_DISABLE_SESSION_NAMING=true

    echo "Step 1: Creating session (should trigger context-length error and compaction)..."
    (cd "$TESTDIR" && "$INDRA_BIN" run --text "hello world" 2>&1) | tee "$OUTPUT"
    ...
    unset INDRA_PROVIDER_SKIP_BACKOFF
    unset INDRA_PROVIDER
    unset INDRA_MODEL
    unset UV_INDEX_URL
  fi
fi
