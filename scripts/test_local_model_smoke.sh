...
show_usage() {
  ...
  echo ""
  echo "Environment:"
  echo "  INDRA_BIN                Optional goose binary path"
...
if [[ -z "${SKIP_BUILD:-}" && -z "${INDRA_BIN:-}" ]]; then
  echo "Building goose..."
  (cd "$REPO_ROOT" && cargo build -p indra-cli --features local-inference --bin indra)
  echo ""
fi

INDRA_BIN="${INDRA_BIN:-$REPO_ROOT/target/debug/goose}"
if [[ ! -x "$INDRA_BIN" ]]; then
  echo "Error: goose binary not found or not executable: $INDRA_BIN"
  exit 1
fi
...
"$INDRA_BIN" lm list | awk 'NR > 2 && $4 == "✓" { print $1 }' > "$EXISTING_MODELS_FILE"
...
cleanup_temp_models() {
  ...
  for model_id in "${TEMP_MODELS[@]}"; do
    if ! "$INDRA_BIN" lm delete "$model_id" >/dev/null 2>&1; then
      cleanup_failed=true
    fi
  done
...
if [[ -n "$MODEL_LIST" ]]; then
  ...
  "$INDRA_BIN" "${SEARCH_ARGS[@]}" > "$SEARCH_JSON"
...
download_once() {
  ...
  if [[ -n "$cache_root" ]]; then
    HF_HUB_CACHE="$cache_root/hub" \
      ...
      "$INDRA_BIN" lm download "$download_id"
  else
    "$INDRA_BIN" lm download "$download_id"
  fi
}
...
run_model() {
  ...
  if [[ "$RUN_TIMEOUT" -eq 0 ]]; then
    INDRA_MODE=auto INDRA_PROVIDER=local INDRA_MODEL="$model_id" \
      "$INDRA_BIN" run --no-profile --text "$INSTRUCTION" 2>&1 | tee "$log_file"
    return "${PIPESTATUS[0]}"
  fi
  ...
  ' \
    "$RUN_TIMEOUT" \
    env INDRA_MODE=auto INDRA_PROVIDER=local INDRA_MODEL="$model_id" \
    "$INDRA_BIN" run --no-profile --text "$INSTRUCTION" 2>&1 | tee "$log_file"
  return "${PIPESTATUS[0]}"
}
...
for row in "${MODELS[@]}"; do
  ...
  if [[ "$KEEP_DOWNLOADS" = false && "$downloaded" = true && "$existed_before" = false ]]; then
    if "$INDRA_BIN" lm delete "$model_id" 2>&1 | tee "$delete_log"; then
