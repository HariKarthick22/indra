{
  description = "INDRA - An AI agent CLI";
  ...
  outputs = { self, nixpkgs, flake-utils, rust-overlay }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        cargoToml = builtins.fromTOML (builtins.readFile ./crates/indra-cli/Cargo.toml);
      ...
      in
      {
        packages.default = pkgs.rustPlatform.buildRustPackage {
          ...
          cargoBuildFlags = [ "--package" "indra-cli" ];
          ...
          checkPhase = ''
            ...
            # Run tests for indra-cli package only
            cargo test --package indra-cli --release
          '';
        ...
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            echo "INDRA development environment"
            echo "Rust version: $(rustc --version)"
            echo ""
            echo "Commands:"
            echo "  nix build           - Build INDRA CLI"
            echo "  nix run             - Run INDRA CLI"
            echo "  cargo build -p indra-cli - Build with cargo"
            echo "  cargo run -p indra-cli   - Run with cargo"
          '';
        };
      }
    );
