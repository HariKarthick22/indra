...
#   $env:INDRA_BIN_DIR  - Directory to which goose will be installed (default: $env:USERPROFILE\.local\bin)
#   $env:INDRA_VERSION  - Optional: specific version to install (e.g., "v1.0.25"). Can be in the format vX.Y.Z, vX.Y.Z-suffix, or X.Y.Z
#   $env:INDRA_PROVIDER - Optional: provider for goose
#   $env:INDRA_MODEL    - Optional: model for goose
#   $env:INDRA_WINDOWS_VARIANT - Optional: Windows package variant to install ("standard" or "cuda")
...
if (-not $env:INDRA_BIN_DIR) {
    $env:INDRA_BIN_DIR = Join-Path $env:USERPROFILE ".local\bin"
}

# Determine release type
$RELEASE = if ($env:CANARY -eq "true") { "true" } else { "false" }
$CONFIGURE = if ($env:CONFIGURE -eq "false") { "false" } else { "true" }
$WINDOWS_VARIANT = if ($env:INDRA_WINDOWS_VARIANT) { $env:INDRA_WINDOWS_VARIANT.ToLowerInvariant() } else { "standard" }

# Determine release tag
if ($env:INDRA_VERSION) {
    # Validate version format
    if ($env:INDRA_VERSION -notmatch '^v?[0-9]+\.[0-9]+\.[0-9]+(-.*)?$') {
        Write-Error "Invalid version '$env:INDRA_VERSION'. Expected: semver format vX.Y.Z, vX.Y.Z-suffix, or X.Y.Z"
        exit 1
    }
    # Ensure version starts with 'v'
    $RELEASE_TAG = if ($env:INDRA_VERSION.StartsWith("v")) { $env:INDRA_VERSION } else { "v$env:INDRA_VERSION" }
} else {
...
if ($WINDOWS_VARIANT -ne "standard" -and $WINDOWS_VARIANT -ne "cuda") {
    Write-Error "Unsupported INDRA_WINDOWS_VARIANT '$WINDOWS_VARIANT'. Expected 'standard' or 'cuda'."
    exit 1
}
...
if (-not (Test-Path $env:INDRA_BIN_DIR)) {
    Write-Host "Creating directory: $env:INDRA_BIN_DIR" -ForegroundColor Yellow
    try {
        New-Item -ItemType Directory -Path $env:INDRA_BIN_DIR -Force | Out-Null
    } catch {
        Write-Error "Could not create directory: $env:INDRA_BIN_DIR"
        Remove-Item -Path $TMP_DIR -Recurse -Force -ErrorAction SilentlyContinue
        exit 1
    }
}
...
$DEST_GOOSE = Join-Path $env:INDRA_BIN_DIR $OUT_FILE
...
foreach ($dll in $DLL_FILES) {
    $DEST_DLL = Join-Path $env:INDRA_BIN_DIR $dll.Name
...
if ($CURRENT_PATH -notlike "*$env:INDRA_BIN_DIR*") {
    Write-Host ""
    Write-Host "Warning: goose installed, but $env:INDRA_BIN_DIR is not in your PATH." -ForegroundColor Yellow
    Write-Host "To add it to your PATH permanently, run the following command as Administrator:" -ForegroundColor Yellow
    Write-Host "    [Environment]::SetEnvironmentVariable('PATH', `$env:PATH + ';$env:INDRA_BIN_DIR', 'Machine')" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Or add it to your user PATH (no admin required):" -ForegroundColor Yellow
    Write-Host "    [Environment]::SetEnvironmentVariable('PATH', `$env:PATH + ';$env:INDRA_BIN_DIR', 'User')" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "For this session only, you can run:" -ForegroundColor Yellow
    Write-Host "    `$env:PATH += ';$env:INDRA_BIN_DIR'" -ForegroundColor Cyan
    Write-Host ""
}
