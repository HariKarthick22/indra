...
copy-binary BUILD_MODE="release":
    ...
    @if [ -f ./target/{{BUILD_MODE}}/indra ]; then \
        ...
        echo "indra CLI binary not found in target/{{BUILD_MODE}}"; \
        exit 1; \
    fi
...
copy-binary-intel:
    @rm -f ./ui/desktop/src/bin/indra
    @if [ -f ./target/x86_64-apple-darwin/release/indra ]; then \
        echo "Copying Intel indra CLI binary to ui/desktop/src/bin..."; \
        rm -f ./ui/desktop/src/bin/indra; \
        cp -p ./target/x86_64-apple-darwin/release/indra ./ui/desktop/src/bin/; \
    else \
        echo "Intel indra CLI binary not found."; \
        exit 1; \
    fi
...
[windows]
copy-binary-windows:
    @powershell.exe -NoProfile -ExecutionPolicy Bypass -Command 'if (Test-Path ./target/x86_64-pc-windows-msvc/release/indra.exe) { \
        Write-Host "Copying Windows binary to ui/desktop/src/bin..."; \
        New-Item -ItemType Directory -Force "./ui/desktop/src/bin" | Out-Null; \
        Remove-Item -Path "./ui/desktop/src/bin/indra.exe" -Force -ErrorAction SilentlyContinue; \
        Copy-Item -Path "./target/x86_64-pc-windows-msvc/release/indra.exe" -Destination "./ui/desktop/src/bin/" -Force; \
    } else { \
...
run-ui-playwright:
    ...
    RUN_DIR="$HOME/indra-runs/$(date +%Y%m%d-%H%M%S)"
    mkdir -p "$RUN_DIR"
    echo "Using isolated directory: $RUN_DIR"
    cd ui/desktop && ENABLE_PLAYWRIGHT=true INDRA_PATH_ROOT="$RUN_DIR" pnpm run start-gui
...
debug-ui:
    @echo "🚀 Starting indra frontend in external ACP backend mode"
    cd ui/desktop && \
    export INDRA_EXTERNAL_BACKEND=true && \
    export INDRA_SERVER__SECRET_KEY="${INDRA_SERVER__SECRET_KEY:-test}" && \
    pnpm install && \
    pnpm run start-gui
...
run-server:
    ...
    INDRA_SERVER__SECRET_KEY="${INDRA_SERVER__SECRET_KEY:-test}" cargo run -p indra-cli --bin indra -- serve --platform desktop --enable-scheduler --host 127.0.0.1 --port 3000

# Check if checked-in ACP artifacts are up-to-date
check-acp-artifacts: generate-acp-types
    #!/usr/bin/env bash
    set -e
    echo "🔍 Checking generated ACP artifacts are up-to-date..."
    if ! git diff --exit-code crates/indra/acp-schema.json crates/indra/acp-meta.json ui/indra-acp-client/src/generated/; then
      echo ""
...
generate-acp-schema:
    ...
    cd crates/indra && cargo run --features code-mode,local-inference,aws-providers,telemetry,otel,rustls-tls,system-keyring --bin generate-acp-schema
    @echo "ACP schema generated: crates/indra/acp-schema.json, crates/indra/acp-meta.json"
...
generate-acp-types: generate-acp-schema
    ...
    cd ui/indra-acp-client && npx tsx generate-schema.ts
...
build-acp-client: generate-acp-types
    ...
    cd ui/indra-acp-client && pnpm run build:ts
    @echo "ACP client package built."

# Generate manpages for the CLI
generate-manpages:
    @echo "Generating manpages..."
    cargo run -p indra-cli --bin generate_manpages
...
[unix]
make-ui-windows:
    @echo "just make-ui-windows requires a Windows host because INDRA Windows releases build the MSVC target. Use .github/workflows/bundle-windows.yml for CI builds."
    @exit 1
...
prepare-release version:
    ...
    @git add \
        ...
        ui/indra-acp-client/package.json \
        ui/indra-acp/package.json \
        ui/indra-binary/*/package.json \
        ui/pnpm-lock.yaml \
        crates/indra-provider-types/src/canonical/data/canonical_models.json \
        crates/indra-provider-types/src/canonical/data/provider_metadata.json
...
win-copy-win profile:
  ...
  if exist ui{{s}}desktop{{s}}src{{s}}bin{{s}}indra.exe del /f /q ui{{s}}desktop{{s}}src{{s}}bin{{s}}indra.exe
...
mcp-conformance-build:
  cargo build -p indra-cli --bin indra --bin mcp_conformance_driver
...
# Example: just mcp-conformance 2025-11-25 all 0.2.0-alpha.10 true crates/indra-cli/tests/mcp-conformance/expected-failures-2025-11-25-0.2.0-alpha.10.yaml
[doc("Run an MCP client conformance suite against INDRA.")]
mcp-conformance version="2025-11-25" suite="all" conformance_version="0.2.0-alpha.10" build="true" baseline="":
  ...
  INDRA_DISABLE_KEYRING=1 npx -y @modelcontextprotocol/conformance@{{conformance_version}} client --command "target/debug/mcp_conformance_driver" --spec-version "{{version}}" --suite "{{suite}}" ${baseline_args[@]+"${baseline_args[@]}"}

build-test-tools:
  cargo build -p indra-test

record-mcp-tests: build-test-tools
  INDRA_RECORD_MCP=1 cargo test --package indra --test mcp_integration_test
  git add crates/indra/tests/mcp_replays/
